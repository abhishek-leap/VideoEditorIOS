...
final class AudioRecorderManager: NSObject {
    ...
    func startRecording(with audioVisualizationTimeInterval: TimeInterval = 0.1666667, completion: @escaping (URL?, Error?) -> Void) {
    ...
    fileprivate func internalStartRecording(with audioVisualizationTimeInterval: TimeInterval) throws -> URL {
        ...
        let path = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0].appendingPathComponent("sounds/"+UUID().uuidString+".m4a")
