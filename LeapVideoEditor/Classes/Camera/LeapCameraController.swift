...
// MARK: todo update name

public protocol LeapCameraControllerUIDelegate: AnyObject {
    ...
    func cameraController(_ controller: LeapCameraController, updatedLenses lenses: [Lens])
    ...
    func cameraControllerRequestedActivityIndicatorShow(_ controller: LeapCameraController)
    ...
    /// - Parameter controller: The camera controller.
    func cameraControllerRequestedActivityIndicatorHide(_ controller: LeapCameraController)

    /// Notifies the delegate that the flash state is on in ring light mode and that the ring light effect should be shown.
    /// - Parameter controller: The camera controller.
    func cameraControllerRequestedRingLightShow(_ controller: LeapCameraController)
    ...
    /// - Parameter controller: The camera controller.
    func cameraControllerRequestedRingLightHide(_ controller: LeapCameraController)

    /// Notifies the delegate that the flash state has changed such that the flash control should be hidden.
    /// - Parameter controller: The camera controller.
    func cameraControllerRequestedFlashControlHide(_ controller: LeapCameraController)

    /// Notifies the delegate that the camera position should be flipped.
    /// - Parameter controller: The camera controller.
    func cameraControllerRequestedCameraFlip(_ controller: LeapCameraController)
    ...
    func cameraController(_ controller: LeapCameraController, requestedHintHideFor lens: Lens)
...
open class LeapCameraController: NSObject, LensRepositoryGroupObserver, LensPrefetcherObserver, LensHintDelegate, LeapMediaPickerViewDelegate, AdjustmentControlViewDelegate
{
    ...
    public weak var uiDelegate: LeapCameraControllerUIDelegate?
    ...
    public var isToneMapAdjustmentAvailable: Bool {
        return cameraKit.adjustments.processor?.isAdjustmentAvailable(ToneMapAdjustment()) ?? false
    }
    ...
    public var isPortraitAdjustmentAvailable: Bool {
        return cameraKit.adjustments.processor?.isAdjustmentAvailable(PortraitAdjustment()) ?? false
    }
    ...
    open func configureLenses(
        ...
        DispatchQueue.global().async { input.startRunning() }
    }
    ...
    public func takePhoto(completion: ((UIImage?, Error?) -> Void)?) {
        ...
        photoCaptureOutput?.capture(
            with: settings,
            outputSize: LeapOutputSizeHelper.normalizedSize(
    ...
    public func disableToneMapAdjustment() {
        guard let toneMapController = toneMapController else {
            return
        }
    ...
    public func disablePortraitAdjustment() {
        guard let portraitController = portraitController else {
            return
        }
    ...
    public func mediaPickerView(_ mediaPickerView: LeapMediaPickerView, selectedAsset: LensMediaPickerProviderAsset) {
...
extension LeapCameraController {
...
extension LeapCameraController {
    ...
    fileprivate func configureRecorder() {
        ...
        recorder = try? Recorder(
            ...
            size: LeapOutputSizeHelper.normalizedSize(
                for: cameraKit.activeInput.frameSize,
                aspectRatio: 10/16,
                orientation: cameraKit.activeInput.frameOrientation)
        )
